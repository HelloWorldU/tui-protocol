...
more than 8 MiB of received binary data.
...
while consumption is held. Callback chunks still overshoot the high watermark,
and OS buffering and total memory remain unmeasured. The longest synchronous
write captures waiting that the zero drain-event count misses.
...
require the development host/token.
