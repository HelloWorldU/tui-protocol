...
export function installLiveCancelCheck(doc: Document): void {
  ...
  const observer = new MutationObserver(() => {
    ...
    armed.checked = false;
    armed.disabled = true;
    result.textContent = "Cancellation sent after visible streaming text; inspect exit, retained content, and Context close below.";
    cancel.click();
    observer.disconnect();
  });
